...
new #[Layout('layouts::admin', ['page_title' => 'Positions'])] class extends Component {
    ...
    public function positions()
    {
        return Position::select(['id', 'position_name', 'department_id', 'is_active'])
            ...
            ->when($this->search, fn($q) => $q->where('position_name', 'like', "%{$this->search}%"))
...
<div>
    <div class="flex flex-1 flex-col gap-8">
        <div class="flex justify-between items-center">
            <flux:input wire:model.live.debounce.300ms="search" class="w-full max-w-md" kbd="⌘K"
                ...
                @foreach ($this->positions() as $post)
                    <flux:table.row :key="$post->id">
                        ...
                        <flux:table.cell class="space-x-4">
                            ...
                            <flux:button class="cursor-pointer" size="sm" wire:click="edit({{ $post->id }})"
                                icon="pencil" />
                            <flux:button class="cursor-pointer" size="sm"
                                wire:click="toggleExpand({{ $post->id }})" icon="cog-6-tooth" />
                            <flux:button icon="trash" size="sm" type="button" variant="danger"
                                wire:click="delete({{ $post->id }})"
                                wire:confirm="Are you sure you want to delete this post?"></flux:button>
                        </flux:table.cell>
                    </flux:table.row>
                    {{-- Expandable row --}}
                    @if (in_array($post->id, $expandedRows))
                        <flux:table.row :key="'expand-'.$post->id">
                            <flux:table.cell colspan="6" class="bg-zinc-50 dark:bg-zinc-800 p-4">
                                <livewire:admin.positions.criteria-panel :position-id="$post->id" :key="'criteria-' . $post->id" />
                            </flux:table.cell>
                        </flux:table.row>
                    @endif
                @endforeach
